...
/*volatile*/ STAR star[MAX_STARS];

private int star_count = 0;
private int star_count_count = 0;



void init_starfield_2d()
{
   int c;
...
void starfield_2d()
{
   int c;
   for (c = 0; c < MAX_STARS; c++) {
      if ((star[c].oy += (cast(int)star[c].z >> 1) + 1) >= SCREEN_H)
         star[c].oy = 0;
   }
}
...
void draw_starfield_2d(BITMAP *bmp)
{
   ...
   for (c = 0; c < MAX_STARS; c++) {
      ...
      putpixel(bmp, x, y, 15 - cast(int)star[c].z);
...
void starfield_3d()
{
   ...
   for (c = 0; c < star_count; c++) {
      ...
      ix = cast(int)(x >> 16) + SCREEN_W / 2;
      iy = cast(int)(y >> 16) + SCREEN_H / 2;
