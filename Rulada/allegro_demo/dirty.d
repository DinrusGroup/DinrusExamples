...
private DIRTY_LIST dirty, old_dirty;
...
void add_to_list(DIRTY_LIST *list, int x, int y, int w, int h)
{
   ...
   if (list.count >= list.size) {
      if (list.size == 0)
         ...
         list.size *= 2;
      list.rect = cast(DIRTY_RECT*)realloc(list.rect,
                                           list.size * DIRTY_RECT.sizeof);
   }
...
extern (C) int rect_cmp(/*const*/ void *p1, /*const*/ void *p2)
{
   DIRTY_RECT *r1 = cast(DIRTY_RECT *) p1;
   DIRTY_RECT *r2 = cast(DIRTY_RECT *) p2;
...
void draw_dirty(BITMAP *bmp)
{
   ...
   if (!gfx_driver.linear)
      qsort(old_dirty.rect, old_dirty.count, DIRTY_RECT.sizeof,
            &rect_cmp);
