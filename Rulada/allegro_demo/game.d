...
static /*volatile*/ uint game_time;
static uint prev_bullet_time;
...
private int fade_intro_item(int music_pos, int fade_speed)
{
   ...
   set_palette(cast(RGB*)datafile[GAME_PAL].dat);
   fade_out(fade_speed);
...
private void draw_intro_item(int item, int size)
{
   BITMAP *b = cast(BITMAP *) datafile[item].dat;
...
extern (C)
private void fps_proc()
{
   volatile fps = frame_count;
   volatile frame_count = 0;
}
...
private void move_everyone()
{
   ...
   if (player_hit) {
      ...
      if (skip_count <= 0) {
         if ((key[KEY_UP]) || (joy[0].stick[0].axis[1].d1)) {
            ...
            if (yspeed < MAX_SPEED) {
               if (yspeed == 0) {
                  play_sample(cast(SAMPLE*)datafile[ENGINE_SPL].dat, 0,
                              PAN(player_x_pos >> SPEED_SHIFT), 1000, true);
               }
               else {
                  /* fade in sample while speeding up */
                  adjust_sample(cast(SAMPLE*)datafile[ENGINE_SPL].dat,
                                yspeed * 64 / MAX_SPEED,
                                PAN(player_x_pos >> SPEED_SHIFT), 1000, true);
               }
               yspeed++;
               ...
               adjust_sample(cast(SAMPLE*)datafile[ENGINE_SPL].dat, 64,
                             PAN(player_x_pos >> SPEED_SHIFT), 1000, true);
            }
            ...
            if (yspeed) {
               yspeed--;
               if (yspeed == 0) {
                  stop_sample(cast(SAMPLE*)datafile[ENGINE_SPL].dat);
               }
               else {
   ...
   if (!player_hit) {
      if ((key[KEY_SPACE] || key[KEY_ENTER] || key[KEY_LCONTROL] || key[KEY_RCONTROL]) ||
         ...
         volatile if (prev_bullet_time + BULLET_DELAY < game_time) {
            bullet = add_bullet((player_x_pos >> SPEED_SHIFT) - 2, SCREEN_H - 64);
            if (bullet) {
               play_sample(cast(SAMPLE*)datafile[SHOOT_SPL].dat, 100, PAN(bullet.x), 1000,
                           false);
...
private void draw_screen(BITMAP *bmp)
{
   int x;
   RLE_SPRITE *spr;
   ...
   switch (animation_type) {
      case ANIMATION_TYPE.DOUBLE_BUFFER:
	 animation_type_str = "double buffered";
	 break;
      case ANIMATION_TYPE.PAGE_FLIP:
	 animation_type_str = "page flipping";
	 break;
      case ANIMATION_TYPE.TRIPLE_BUFFER:
	 animation_type_str = "triple buffered";
	 break;
      case ANIMATION_TYPE.DIRTY_RECTANGLE:
   ...
   if ((ship_burn) && (ship_state < EXPLODE_FLAG)) {
      spr = cast(RLE_SPRITE *) datafile[ENGINE1 + (retrace_count / 4) % 7].dat;
   ...
   if (ship_state >= EXPLODE_FLAG)
      ...
      spr = cast(RLE_SPRITE *) datafile[ship_state].dat;
   ...
   volatile if (fps)
      sprintf(score_buf.ptr, "Lives: %d - Score: %d - (%s, %d fps)",
              ship_count, score, animation_type_str, fps);
   else
      sprintf(score_buf.ptr, "Lives: %d - Score: %d", ship_count, score);

   textout_ex(bmp, font, score_buf.ptr, 0, 0, 7, 0);
   if (animation_type == ANIMATION_TYPE.DIRTY_RECTANGLE)
      dirty_rectangle(0, 0, text_length(font, score_buf.ptr), 8);
...
private void draw_score(BITMAP *bmp)
{
   ...
   clear_bitmap(b);
   textout_centre_ex(b, cast(FONT*)datafile[END_FONT].dat, "GAME OVER", 80, 50, 2, 0);
   sprintf(score_buf.ptr, "Score: %d", score);
   textout_centre_ex(b, cast(FONT*)datafile[END_FONT].dat, score_buf.ptr, 80, 82, 2, 0);
...
void play_game()
{
   ...
   play_midi(cast(MIDI*)datafile[GAME_MUSIC].dat, true);
   clear_keybuf();
 ...
 skip:
   ...
   while (!esc) {
      ...
      volatile while (prev_update_time < game_time) {
         if (dead) {
            move_score();
         }
         else {
