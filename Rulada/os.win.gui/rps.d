...
class GameForm: Form
{
	...
	void killConnectedSocket()
	{
		...
		{
			mysock.close();
			os.win.gui.socket.unregisterEvent(mysock);
			mysock = null;
		}
	...
	void mainBtnClick(Object sender, EventArgs ea)
	{
		...
		else
		{
			...
			i = std.string.find(servaddr, ':');
			if(i != -1)
			{
				servport = std.conv.toUshort(servaddr[i + 1 .. servaddr.length]);
			...
			try
			{
				...
				os.win.gui.socket.registerEvent(mysock, // Socket to get events for.
	...
	void netEvent(Socket sock, EventType type, int err)
	{
		switch(type)
		{
			case EventType.CONNECT:
				...
				if(err)
				{
					status.text = "Unable to connect. Error #" ~ std.string.toString(err) ~ ".";
					sock.close();
					mysock = null;
